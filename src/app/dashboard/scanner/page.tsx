...
import { Search, Globe, Loader2, CheckCircle2, ArrowRight, ShieldCheck, Sparkles } from "lucide-react";
import { motion } from "framer-motion";

function ScannerForm() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const { setActiveScan } = useAuditStore();
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="p-8 rounded-3xl border border-slate-800 bg-slate-900/60 glass-panel shadow-2xl space-y-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={isScanning}
                required
                className="w-full pl-12 pr-4 py-3 rounded-2xl border border-slate-700 bg-slate-950 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:outline-none"
              />
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all shadow-xl shadow-blue-600/30 flex items-center justify-center space-x-2 disabled:opacity-50"
          >
            {isScanning ? (
              <>
        ...
        {isScanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pt-4 border-t border-slate-800">
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 font-mono text-xs text-slate-300 leading-relaxed">
